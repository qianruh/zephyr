...
#define ACCEL_Q31_SHIFT 6
#define GYRO_Q31_SHIFT 15 

static q31_t accel_sensor_value_to_q31(struct sensor_value *val)
{
	int shift = ACCEL_Q31_SHIFT;
	int64_t micro_g = sensor_ms2_to_ug(val);

	return (q31_t)((micro_g * INT32_MAX / 1000000) >> shift);
}

static void accel_q31_to_sensor_value(q31_t q, struct sensor_value *val)
{
	int shift = ACCEL_Q31_SHIFT;
	int64_t shifted = q << shift;
	int64_t micro_g;

	micro_g = FIELD_GET(GENMASK64(31 + shift, 31), shifted) * 1000000 +
		((FIELD_GET(GENMASK64(30, 0), shifted) * 1000000) / INT32_MAX);
...
{
	int shift = GYRO_Q31_SHIFT;
	int64_t micro_deg = sensor_rad_to_10udegrees(val) * 10;

	return (q31_t)((micro_deg * INT32_MAX / 1000000) >> shift);
}
...
static void gyro_q31_to_sensor_value(q31_t q, struct sensor_value *val)
{
	...
	micro_deg = FIELD_GET(GENMASK64(31 + shift, 31), shifted) * 1000000 +
		((FIELD_GET(GENMASK64(30, 0), shifted) * 1000000) / INT32_MAX);
	...
	if (enable != ctx->data_ready_enabled) {
		...
		if (!ret) {
			if (!sensor_trigger_set(ctx->hw_dev, &ctx->trig,
				...
				sensing_sensor_set_data_ready(ctx->dev, !enable);
